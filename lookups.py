...
def _pick_cols(df: pd.DataFrame):
    ...
    code_col = next((lower[k] for k in ["code","occurrence_code","occurrence_no","phase_no","phase_code","ct_code","code_iaids"] if k in lower), None)
    meaning_col = next((lower[k] for k in ["meaning","desc","description","occurrence","phase","phase_meaning"] if k in lower), None)
    # fallbacks
    if code_col is None:
        for c in df.columns:
...
def build_phase_and_event_lookups_from_seq(seq: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if code_col is None or meaning_col is None:
        return pd.DataFrame(columns=["phase_no","phase_meaning"]), pd.DataFrame(columns=["eventsoe_no","event_meaning"])
