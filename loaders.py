...
from config import (
    DATA, DATE_FORMATS, EVENTS_CSV, FINDINGS_CSV, AIRCRAFT_CSV, EVENTS_SEQUENCE_CSV,
    EVENTS_COLS, FINDINGS_COLS, AIRCRAFT_COLS, SEQ_COLS
)
from normalize import parse_flexible_datetime, normalize_make_model

def read_events(path: Path = DATA) -> pd.DataFrame:
    df = pd.read_csv(
        EVENTS_CSV, usecols=EVENTS_COLS,
...
def read_findings(path: Path = DATA) -> pd.DataFrame:
    return pd.read_csv(
        FINDINGS_CSV, usecols=FINDINGS_COLS,
        dtype={"ev_id":"string","Aircraft_Key":"Int64","finding_no":"Int64","finding_code":"Int64",
               "finding_description":"string","category_no":"Int64","subcategory_no":"Int64",
               "section_no":"Int64","subsection_no":"Int64","modifier_no":"Int64","Cause_Factor":"string"},
        low_memory=False
    )

def read_aircraft(path: Path = DATA) -> pd.DataFrame:
    df = pd.read_csv(
        AIRCRAFT_CSV, usecols=AIRCRAFT_COLS,
        dtype={"ev_id":"string","Aircraft_Key":"Int64","acft_make":"string","acft_model":"string"},
        low_memory=False
    )
...
def read_events_sequence(path: Path = DATA) -> pd.DataFrame:
    ...
    df = df[keep].copy()
    for col in ["Occurrence_No","phase_no","eventsoe_no","Defining_ev","Aircraft_Key"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype("Int64")
    if "ev_id" in df: df["ev_id"] = df["ev_id"].astype("string")
