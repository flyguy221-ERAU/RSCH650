...
def main():
    ...
    spec = FilterSpec(years=(args.start, args.end), include_far_parts=set(args.parts), exclude_rotorcraft=True)
    ...
    try:
        ...
        stats = {"chi2": float(chi2), "p": float(p), "dof": int(dof), "expected": expected.tolist()}
    ...
    if args.format == "csv":
        ...
        with open(f"{out}/chisq.json","w") as f: json.dump(stats, f, indent=2)
        with open(f"{out}/logit_summary.txt","w") as f: f.write(summ)
    else:
