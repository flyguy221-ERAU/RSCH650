...
def build_phase_numeric_lookup() -> pd.DataFrame:
    ...
    if m.empty:
        m = dd[(dd["Table_l"]=="occurrences") & (dd["Column_l"]=="phase_of_flight")][["code_iaids","meaning"]].dropna()
    if m.empty:
        return pd.DataFrame(columns=["code_int","meaning"])
    code = m["code_iaids"].str.extract(r"(\d+)$")[0].fillna(m["code_iaids"].str.extract(r"(\d+)")[0])
    lk = pd.DataFrame({"code_int": pd.to_numeric(code, errors="coerce").astype("Int64"),
                       "meaning": m["meaning"]})
